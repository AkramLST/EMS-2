...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[Sync Profile Images] Starting sync process...');
    ...
    try {
      ...
      console.log('[Sync Profile Images] No uploads directory found');
      ...
      try {
        ...
        if (employee) {
          if (!employee.profileImage) {
            ...
            console.log(`[Sync Profile Images] Updated ${employeeId} with ${imageUrl}`);
            syncedCount++;
            results.push({
              employeeId,
          ...
          console.log(`[Sync Profile Images] No employee found for ${employeeId}`);
          results.push({
            employeeId,
            imageUrl,
        ...
        console.error(`[Sync Profile Images] Error processing ${employeeId}:`, error);
        results.push({
          employeeId,
          imageUrl,
          status: 'error',
    ...
    console.log(`[Sync Profile Images] Completed. Synced ${syncedCount} images.`);

    return NextResponse.json({
      message: `Successfully synced ${syncedCount} profile images`,
      synced: syncedCount,
      total: files.length,
      results
    });
